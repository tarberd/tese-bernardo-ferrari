...
fn translate_term2(term2: Parse) -> Option<(BinOp, Ast)> {
  ...
  \end{minipage}
  \caption{
    A tree transformation from the \code{Term} and \code{Term2} nodes in Figure~\ref{fig:abstract_syntax_adt_a} into the Ast nodes in Figure~\ref{fig:abstract_syntax_adt_b}.
...
fn translate_factor(factor: Parse) -> Ast {
  ...
  \end{minipage}
  \caption{
    A tree transformation from the \code{NestedExpr}, \code{Number}, and \code{Id} nodes in Figure~\ref{fig:abstract_syntax_adt_a} into the Ast nodes in Figure~\ref{fig:abstract_syntax_adt_b}.
...
We can easily derive an ADT definition for the abstract syntax of $\Gamma$, \code{term}, and \code{Type} presented in the simply typed lambda calculus defined by grammar~\eqref{eq:simply_typed_lc} as show by Figure~\ref{fig:typeof_adt}.
...
Given the ADT definitions of \code{Context}, \code{Term}, and \code{Type}, we can now define the \code{typeof} function as show in Figure~\ref{fig:typeof_impl}.
The definition of \code{typeof} in Figure~\ref{fig:typeof_impl} is directly derived from the inference rules from Figure~\ref{fig:simpli_typed_type_rel} and the inference rules from~\ref{fig:sample_term_ast_tyrel} with added context propagation.
...
The intermediate code generation will take as input the abstract syntax tree generated from semantic analisys and output an intermediate representation (IR).
...
In practive, compilers may construct a sequence of many intermediate syntactical representations until reaching the target machine executable code~\cite{Aho:2006:CPT:1177220}.
Each one suitable for the kind of analisys and operations it is executed uppon.

We have already described how to evaluate a concrete parse tree into an abstract syntax tree in Section~\ref{sec:syntax:subsec:abstrac_syntax_trees} and the process of creating tree traversals algorithms in order to compute a new syntactical representation continues into evaluating an abstract syntax tree into an IR suitable for code optimization and target code generation.
