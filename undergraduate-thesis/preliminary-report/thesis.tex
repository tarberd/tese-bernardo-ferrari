...
\end{align*}
now we can prove if $\code{2 * 3 == 42}: \bnfters{Bool}$ by the following sequence of inferences
\begin{equation*}
  \inferrule*[]{
    \inferrule*[
...
\end{equation*}
but if we try to prove something wrong like $\code{2 * true}: \bnfters{Num}$ we will quickly find that there are no rules that allow us to reach the type relation axioms.

But what happens if we try to prove $\code{a + 5} : \bnfters{Num}$ for example?
We currently have no rules that allow us to know infer the type of \code{a}.
