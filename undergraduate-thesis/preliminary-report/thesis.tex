...
The abstract syntax for terms and types presented by the authors is summarized

\begin{figure}[ht]
  \begin{minipage}{\textwidth}
    \begin{alignat*}{4}
      \astprod{term}{c}              & \quad & \textit{Constants}    \\
      \astmore{var}                    & \quad & \textit{Variables}    \\
      \astmore{let var = term in term} & \quad & \textit{Let Bindings} \\
      \astmore{$\lambda$x.term}      & \quad & \textit{Functions}    \\
      \astmore{term var}               & \quad & \textit{Applications} \\
      \astmore{term:type}            & \quad & \textit{Applications} \\
    \end{alignat*}
  \end{minipage}
  \caption{
    Abstract syntax for refinement predicates and constraints.
