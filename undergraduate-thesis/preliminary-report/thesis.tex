...
\newcommand{\token}[1]{$\langle\texttt{#1}\rangle$}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    child { node {$\bnfters{let}$} }
    child {
        node {$\bnfvars{Pattern}$}
        child {
            node[align=center] {$\bnfters{id}$}
            child {
                node {a}
...
nonterminals have the syntactic variables as normal words, e.g. $\bnfvars{Expr}$;
...
\begin{figure}[ht]
  \centering
  \begin{minipage}{0.5\textwidth}
    \begin{minted}[breaklines, linenos, fontsize=\footnotesize]{text}
Expr
  Term
    Factor
      Number "5"
...
% Suppose the following abstract syntax for terms
% \begin{alignat}{2}
%   \bnfprod{term}{\bnfvar{term} \bnfvar{Expr2}}              \label{eq:fat_expr_grammar_expr}  \\
%   \bnfprod{Expr2}{\bnfter{+}\bnfvar{Term} \bnfvar{Expr2}}   \label{eq:fat_expr_grammar_expr2} \\
%   \bnfmore{\bnfter{-}\bnfvar{Term}\bnfvar{Expr2}}           \label{eq:fat_expr_grammar_expr3} \\
%   \bnfmore{\ \epsilon}                                      \label{eq:fat_expr_grammar_expr4} \\
%   \bnfprod{Term}{\bnfvar{Factor} \bnfvar{Term2}}            \label{eq:fat_expr_grammar_term}  \\
...
We introduce the syntactic form for $\bnfvars{T}$ as
...
\begin{align*}
  \bnfters{true} : \bnfters{Bool}
   &  & \bnfters{false} : \bnfters{Bool}
   &  & \bnfters{number} : \bnfters{Nat}
\end{align*}
\begin{align*}
  \inferrule*[right=num-arith]{
    \bnfvars{t}_1 : \bnfters{Num}
  \\ \bnfvars{t}_2 : \bnfters{Num}
  \\ \bnfvar{op} \in \bnfvars{oparit}
  }{
    \bnfvars{t}_1 \bnfvar{op} \bnfvar{t}_2 : \bnfters{Num}
  }
\end{align*}
\begin{align*}
  \inferrule*[right=num-compare]{
    \bnfvars{t}_1 : \bnfters{Num}
  \\ \bnfvars{t}_2 : \bnfters{Num}
  \\ \bnfvar{op} \in \bnfvars{opcomp}
  }{
    \bnfvars{t}_1 \bnfvar{op} \bnfvar{t}_2 : \bnfters{Bool}
  }
\end{align*}
\begin{align*}
  \inferrule*[right=bool-arith]{
    \bnfvars{t}_1 : \bnfters{Bool}
  \\ \bnfvars{t}_2 : \bnfters{Bool}
  \\ \bnfvar{op} \in \bnfvars{opbool}
  }{
    \bnfvars{t}_1 \bnfvar{op} \bnfvar{t}_2 : \bnfters{Bool}
  }
\end{align*}
\begin{align*}
  \inferrule*[right=if]{
    \bnfvars{t}_1 : \bnfters{Bool}
  \\ \bnfvars{t}_2 : \bnfvars{T}
  \\ \bnfvars{t}_3 : \bnfvars{T}
  }{
    \bnfters{if} \bnfvar{t}_1 \bnfter{then} \bnfvar{t}_2 \bnfter{else} \bnfvar{t}_3 : \bnfvars{T}
  }
\end{align*}
now we can prove if $\texttt{2 * 3 == 42}: \bnfters{Bool}$ by the following sequence of inferences
\begin{equation*}
  \inferrule*[]{
    \inferrule*[
      leftskip=2em,rightskip=2em,vdots=1.5em]
    {
      \bnfvars{2} : \bnfters{Num}  \bnfter{*} \bnfvar{3} : \bnfters{Num}
      \\\bnfvars{3} : \bnfters{Num}
      \\ \bnfters{*} \in \bnfvars{oparit}
    }{
      \bnfvars{2} \bnfter{*} \bnfvar{3} : \bnfters{Num}
    }
    \\ \bnfvars{42} : \bnfters{Num}
    \\ \bnfters{==} \in \bnfvars{opcomp}
  }{
    \\ \bnfvars{2} \bnfter{*} \bnfvar{3} \bnfter{==} \bnfvar{42}: \bnfters{Bool}
  }
\end{equation*}
but if we try to prove something wrong like $\texttt{2 * true}: \bnfters{Num}$ we will quickly find that there are no rules that allow us to reach the type relation axioms.

But what happens if we try to prove $\texttt{a + 5} : \bnfters{Num}$ for example?
We currently have no rules that allow us to know infer the type of \texttt{a}.
...
The context $\Gamma$, also called \textit{typing context}, is a set of assumptions of the form $\bnfters{id} : \bnfvars{Type}$~\cite{pierce2002types}.
It is described by the syntactic form
\begin{equation}~\label{eq:sample_term_ast_bool_g}
  \begin{alignedat}{2}
    \bnfprod{$\Gamma$}{\ \Gamma \bnfter{,} \bnfter{id} : \bnfvars{Type}} \\
    \bnfmore{\bnfter{$\emptyset$}}
  \end{alignedat}
\end{equation}

The new three-place typing relation is written $\Gamma \vdash \bnfvars{term} : \bnfvars{Type}$.
