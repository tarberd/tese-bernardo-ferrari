...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
and second, we define and \textit{evaluation relation} on terms, written \evalr{term}{$\code{term}_1$} meaning \code{term} evaluates to $\code{term}_1$ in on step, as
\begin{align*}
   & \evalr{if true then $\code{t}_1$ else $\code{t}_2$}{$\code{t}_1$} \tag{Eval-IfTrue} \\
   & \evalr{if false then $\code{t}_1$ else $\code{t}_2$}{$\code{t}_2$} \tag{Eval-IfFalse}
\end{align*}
\begin{equation*}
  \inferrule{
    \evalr{$\code{t}_1$}{$\code{t}_4$}
  } {
    \evalr{
      if $\code{t}_1$ then $\code{t}_2$ else $\code{t}_3$
    }{
      if $\code{t}_4$ then $\code{t}_2$ else $\code{t}_3$
    }
  }\tag{Eval-If}
\end{equation*}
\begin{equation*}
  \inferrule{
    \evalr{$\code{t}_1$}{$\code{t}_2$}
  } {
    \evalr{
      succ $\code{t}_1$
    }{
