...
The terms and their respective inference rules are:
...
\begin{itemize}
  \item Constants \code{c} synthesize their primitive type denoted by \code{prim(c)} as formalized by the rule Syn-Const.
  ...
  \item Variables \code{x} synthesize the type \code{t} if $\typer{x}{t} \in \Gamma$ through the use of the rule Syn-Var.

  \item Applications \code{e x} synthesize the output type of the function type synthesized for the term \code{e} with its input binder substituted by the real argument variable \code{x} through rule Syn-App.
