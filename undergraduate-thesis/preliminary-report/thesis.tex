...
\begin{figure}[ht]
  ...
  \begin{equation*}
    \inferrule*[right=Chk-If]{
      \qquad \qquad \qquad \quad \ \ \code{y} \notin \Gamma \\
      \checkcxr{}{x}{bool\{x|true\}} \\\\
...
\begin{itemize}
  \item Constant terms \code{c} synthesize their primitive type denoted by \code{prim(c)} as formalized by the rule Syn-Const.
        ...
        \begin{align*}
          \code{prim(let)} & = \fnty{x:int}{\fnty{y:int}{\code{bool\{z|z == x <= y\}}}} \\
          \code{prim(geq)} & = \fnty{x:int}{\fnty{y:int}{\code{bool\{z|z == x > y\}}}}  \\
        \end{align*}
