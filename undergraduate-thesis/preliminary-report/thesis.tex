...
\data{10 de Junho de 2024} % ou \today
...
\begin{abstract}
  ...
  \vspace{\baselineskip}
  \textbf{Keywords:} Programming Language\@. Refinement Types\@. Intermidiate Code Representation\@. Compiler\@. LLVM\@. LLVM-IR\@.
\end{abstract}
...
In this type system, a call to \code{size(x)} returns a value $s$ of type \codett{Nat} constrained to a single value equal to the length of \codett{x}; hence, the type of $s$ constrains $s$ to the exact length of \codett{x}.
Furthermore, a call to \code{get(x, i)} requires the index \codett{i} to be within the bounds of \codett{x}.
...
In~\cref{chapter:proposal}, we describe the proposed \textit{Ekitai} language, its frontend implementation, and major results of the research.
Than, in~\cref{ch:conclusion}, we talk about future work ideas and bring closure to the thesis.
...
For simple languages like~\eqref{eq:sample_term_ast} the state of the abstract machine can be defined by just a \code{term} and its behavior by a \textit{transition function} that for each \code{term} either performs an evaluation step, or halts and announces that the evaluation has ended~\cite{pierce2002types}.
...
\begin{equation}\label{eq:sample_ast_values}
  ...
  \end{aligned}
\end{equation}
and second, we define and \textit{evaluation relation}, or \textit{judgments}, on terms, written
\begin{equation*}
  \evalr{term}{$\code{term}_1$}
\end{equation*}
meaning \code{term} evaluates to $\code{term}_1$ in one step~\cite{pierce2002types}.
...
The evaluation relation for~\eqref{eq:sample_term_ast} is the smallest relation defined by the inference rules in~\cref{fig:sample_term_ast_opsem}.
Using the evaluation relations from~\cref{fig:sample_term_ast_opsem} we can evaluate the following program
\begin{equation}\label{eq:eval_step1}
  \code{if iszero 0 then succ 0 else false}
\end{equation}

By looking at~\cref{fig:sample_term_ast_opsem}, we see that the only inference rule we can apply is Eval-If.
When applying this rule to~\eqref{eq:eval_step1} we get
\begin{equation*}
  \inferrule*[right=Eval-If]{
...
In this section we will be discussing a variation of what \textcite{pierce2002types} calls \textit{Pure simply typed lambda-calculus}.
...
In order to be capable of expressing the types of variables we need to extend the typing relation from a two-place relation into a three-place relation adding a context $\Gamma$~\cite{pierce2002types}.
...
We can easily construct Abstract Data Types (ADTs) for the abstract syntax of $\Gamma$, \code{term}, and \code{Type} presented in the simply typed lambda calculus defined by grammar~\eqref{eq:simply_typed_lc} as show by~\cref{fig:typeof_adt}.
...
enum Term {
  ...
  \end{subfigure}
  \caption{
    Abstract Data Types for the context $\Gamma$~(a), \code{Type}~(b), and \code{term}~(c) from the abstract syntax defined in grammar~\eqref{eq:simply_typed_lc} written in Rust.
...
fn typeof(cx: Context, t: Term) -> Type {
  ...
  \end{minipage}
  \caption{
    An implementation of the \code{typeof} function for typechecking the simply typed lambda calculus described in~\cref{sec:semantic:purelambda} written in Rust.
...
\section{Extending the simply typed lambda calculus with refinements}\label{sec:simply_typed_lambda_calculus_with_refinements}
...
\end{equation*}
and the operation $\code{p}_1\bowtie\code{p}_2$ was added to represent all interpreted operators the logic of predicates can be extended with in order to use features from Satisfiability Modulo Theories (SMT) decidable logics (e.g., set operations)~\cite{jhala2020tutorial}.
...
As explored in section \cref{chapter:background:sec:lexical} we define the lexical aspects of the Ekitai programming language presented in~\cref{fig:ekitai_tokens_helper,fig:ekitai_tokens}.
...
The Ekitai's grammar was built with a handwritten Pratt parser in mind \cite{pratt1973operatorprecedence}.
As explored in section \cref{chapter:background:sec:syntax} we will be presenting Ekitais syntax.
...
From the grammar in~\cref{fig:ekitai_syntax_toplevel}, we are able to see the structure of the top level constructs.
For example, the following program illustrates a \bnfvar{SourceFile} with two \bnfvar{Item} in the \bnfvar{ItemList}:
\begin{center}
  \begin{minipage}{0.8\textwidth}
    \begin{rustcode}
type SomeTypeName {
...
\end{rustcode}
  \end{minipage}
\end{center}
When translating the Ekitai's parse tree to Ekitai's we use the \bnfvar{Path} variable to index the constructors inside the \code{type} definition and functions names inside modules, for example the path
\begin{equation*}
...
\begin{equation*}
  \code{SomeTypeName}
\end{equation*}
and respectively for functions of different source files.
The \bnfvar{Path} variable is defined on~\cref{fig:ekitai_syntax_terms} as part of Ekitai's grammar for terms and is used to uniquely identify functions and types of the Ekitai language.
...
The techniques used by Ekitai's Pratt parser were explored in~\cref{chapter:background:sec:syntax}.
If the reader wants to know the specifics of the implementation we guide to source files of the modules \code{syntax} and \code{parser} of Ekitai's source code at~\cref{appendix:ekitai_github}.

After construction of the CST datastructure containg the parse tree, Ekitai's front-end translates its CST to an intermediate AST.
The intermediate AST is derived of the lambda calculus with refinements presented in~\cref{ch:refinement_types}.
The techniques employed in this translation were explored in~\cref{sec:syntax:subsec:abstrac_syntax_trees}.
If the reader wishes to explore the details of the AST implementation we guide to source files of the module \code{hir}, submodule \code{semantic\_ir}, of Ekitai's source code at~\cref{appendix:ekitai_github} which contains all structs for dealing with Ekitai's higher level intermediate representation.
We will be defining Ekitai's AST formally and exploring it's type checking algorithms in~\cref{sec:ekitai_type_system}.

\section{The Ekitai's type system}\label{sec:ekitai_type_system}

Ekitai's type-checker aimins to provide developers with a powerful yet intuitive type system by enabling the use of predicates to prove properties on terms of the language.
Drawing from the theoretical foundation detailed in \cref{ch:refinement_types}, this type system is an implementation of refinement types for a subset of lambda calculus.

\subsection{Abstract Syntax Tree (AST)}

Central to understanding Ekitai's type system is its AST (see~\cref{fig:ekitai_ast}).
The Ekitai's AST is a derivation of the abstract syntax presented back in~\cref{sec:simply_typed_lambda_calculus_with_refinements}, which significantly eases the process of type-checking since we can use the same inference rules described to type-check programs that where written in Ekitai's Syntax.
...
\begin{figure}[!htb]
  \begin{alignat*}{2}
    ...
    \astprod{Name}{id} \\
    \astprod{Path}{Name Path \bnfor{$\epsilon$}} \\
\\
...
\\
    \astprod{$\Gamma$}{$\emptyset$}        \\
    \astmore{$\Gamma$, \typer{Path}{Type}}    
  \end{alignat*}
  \caption{
    Ekitai's AST derived from the lambda calculus with refinements from~\cref{sec:simply_typed_lambda_calculus_with_refinements}.
  }\label{fig:ekitai_ast}
\end{figure}

We employed two derivations in Ekitai's AST compared to the lambda calculus with refinements detailed in~\cref{sec:simply_typed_lambda_calculus_with_refinements}:
\begin{itemize}
  \item First, we added toplevel items for type and funciton definitions;
  \item Seccond, we removed the function term of the language.
\end{itemize}
The first one enables us to bring everything into context before typechecking the terms inside functions.
While the seccond one was employed to simplify the subsequent translation into LLVM-IR by avoiding the need to create unamed closure objects and deal with capturing variables from the function's context.  

Next we will explore some key aspects of Ekitai's AST and typechecker implementations.
Examples where taken from module \code{hir}, submodules \code{liquid} and \code{check}, found in~\cref{appendix:ekitai_github}
...
\begin{itemize}
  \item The \bnfvar{RefinedBase} is a structure that encapsulates the base type along with a refinement predicate.
  \item The \bnfvar{DependentFunction} represents functions that possess a dependency on a specific parameter type.
  \item \bnfvar{RefinedType} serves as an overarching enum, capturing the essence of both basic and functional types.
  \item The differentiation between abstract data types, function definitions, pointers, and scalars is captured by the \bnfvar{Type} enum.
\end{itemize}
...
After we make sure that the program is free from type errors we are able to bring the AST together with all type metadata brought from Ekitai's typechecker we can finally begin the AST's translation to LLVM-IR.
The examples in this section where taken from module \code{codegen} found in~\cref{appendix:ekitai_github}.

The \code{fold\_binary\_expression} function is a pivotal part of this translation process.
This function handles the translation of binary expressions in the Ekitai language into their LLVM equivalents.
The function takes in parameters including a potential pointer for indirect values, the operator involved, and the left-hand side (lhs) and right-hand side (rhs) terms.
Inside the function, \code{fold\_expression} is recursively called on both lhs and rhs to generate their LLVM-IR counterparts.
The function then matches the operator to generate the appropriate LLVM instruction, such as \code{build\_int\_add} for addition operations.
...
Similarly, the \code{fold\_call\_expression} function translates function call expressions.
It first retrieves the type of the callee and ensures it is a callable type.
The function then allocates space for the return value if it is not directly returned via registers.
The actual call instruction is created using \code{build\_call}, and the function handles whether the return value is direct or indirect.
...
The \code{fold\_if\_expression} function handles the translation of conditional expressions.
It constructs LLVM basic blocks for the then and else branches as well as the merge block.
It creates the conditional and unconditional branch instructions to ensure proper control flow and uses a Phi node to merge the values from both branches if necessary.
...
These examples illustrate the meticulous process of translating high-level Ekitai constructs into LLVM-IR, ensuring that the generated code is both efficient and accurate.
The careful handling of direct and indirect values, precise control flow management, and type-aware function calls are fundamental to producing robust and performant intermediate code.
...
\begin{itemize}
  \item The \code{CodeGenTypeCache} structure is responsible for caching LLVM types to ensure efficient reuse throughout the code generation process.
\begin{rustcode}
...
\item The \code{CodeGenFunctionInfoCache} structure manages information related to function definitions in the code generation process.
\begin{rustcode}
...
\section{Examples and intermediate constructs}

In this section we will be breathly discussing working examples of the Ekitai compiler.

\subsection{Identity function with refinements}
...
Ekitai's type checker will generate the following context to start entailment for the return expression \code{x}:
...
\subsection{Absolute value function with refinements}
...
Ekitai's type checker will check for entailment in multiple points durring typechecking.
In the following code, we can see the starting context for the body of the \code{abs\_liquid} function:
...
During typecheking of the \code{abs\_liquid} function's body, there will be multiple calls to entailment.
We can see the calls made in the following code:

\begin{rustcode}
Solver:
(declare-fun x () Int)
(declare-fun __arg0 () Int)
(declare-fun __arg1 () Int)
(declare-fun __arg2 () Bool)
(declare-fun ret () Bool)
(assert (and true
     (= ret (> __arg0 __arg1))
     (= ret __arg2)
     (= __arg2 (> __arg0 __arg1))
     (= __arg1 0)
     true
     (= __arg0 x)
     true
...
\begin{itemize}
    \item \textbf{Predicate-Based Optimizations}: Leveraging the predicates defined in refinement types for advanced optimizations in the Abstract Syntax Tree (AST) and LLVM code generation phase. This could involve:
    \begin{itemize}
        \item \textbf{Bounds Checking Elimination}: Using predicates to prove at compile time that certain array accesses are always within bounds, thus eliminating the need for runtime checks.
        \item \textbf{Nullability Checks Removal}: Ensuring through predicates that certain variables can never be null, allowing the removal of redundant null checks.
        \item \textbf{Specialized Code Paths}: Generating specialized code paths for different refined types, optimizing the code for specific cases known at compile time.
    \end{itemize}
    \item \textbf{Extended Refinement Type System}: Enhancing the refinement type system to support more complex constraints and type relationships could provide additional safety guarantees. This could include support for dependent types or more sophisticated constraint solvers.
    \item \textbf{Improved Error Reporting}: Enhancing the compiler's error reporting mechanisms to provide more informative and user-friendly messages can significantly improve the developer experience. This includes better integration of refinement type errors and suggestions for fixing them.
    \item \textbf{Benchmarking and Real-World Testing}: Conducting extensive benchmarking and real-world testing with a variety of applications will help identify performance bottlenecks and areas for further optimization. This empirical data can guide future enhancements to both the compiler and the language itself.
    \item \textbf{User and Community Feedback}: Actively seeking feedback from users and the developer community can provide valuable insights into the practical usability of Ekitai. This feedback can drive the prioritization of new features and improvements based on actual user needs.
\end{itemize}
...
\chapter{Ekitai's implementation}\label{appendix:ekitai_github}
The Ekitai's implementation in Rust spans more than 12000 lines of code across 121 files and hence will not be fully anexed to this document.
Please refere to the code's repository hosted at \url{https://github.com/tarberd/ekitai/}.

\section{Submodule hir::liquid}
